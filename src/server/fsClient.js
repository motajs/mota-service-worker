(function () {
    ...
    let _http = function (type, url, formData, success, error, mimeType, responseType) {
    ...
    let postsomething = function (data, _ip, callback) {
    ...
    fs.readFile = function (filename, encoding, callback) {
        ...
        if (encoding == 'utf-8') {
            ...
            data += 'type=utf8&';
            data += 'name=' + filename;
            postsomething(data, './fs/readFile', callback);
            return;
        }
        if (encoding == 'base64') {
            //读二进制文件
    ...
    fs.writeFile = function (filename, datastr, encoding, callback) {
        ...
        if (encoding == 'utf-8') {
            ...
            data += 'name=' + filename;
            data += '&value=' + datastr;
            postsomething(data, './fs/writeFile', callback);
            return;
        }
        if (encoding == 'base64') {
            //写二进制文件
    ...
    fs.writeMultiFiles = function (filenames, datastrs, callback) {
        postsomething('name=' + filenames.join(';') + '&value=' + datastrs.join(';'), './fs/writeMultiFiles', callback);
    }
    ...
    fs.readdir = function (path, callback) {
        ...
        let data = '';
        data += 'name=' + path;
        postsomething(data, './fs/listFile', function (err, data) {
            try {
    ...
    fs.mkdir = function (path, callback) {
        ...
        let data = '';
        data += 'name=' + path;
        postsomething(data, './fs/makeDir', callback);
        return;
    }
    ...
    fs.moveFile = function (src, dest, callback) {
        ...
        postsomething(data, './fs/moveFile', callback);
        return;
    }
    ...
    fs.deleteFile = function (path, callback) {
        ...
        let data = '';
        data += 'name=' + path;
        postsomething(data, './fs/deleteFile', callback);
        return;
    }
})();